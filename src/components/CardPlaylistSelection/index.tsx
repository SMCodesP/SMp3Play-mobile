...
export const CardPlaylistSelection: React.FC<CardPlaylistSelection> = ({ item, video }) => {
  ...
  return (
    <TouchableWithoutFeedback onPress={handleAdd} touchSoundDisabled={true}>
      <RectButton
        style={styles.modalContainerPlaylist}
        rippleColor={darken(0.1, colors.comment)}
      >
        <View style={{ flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "space-between" }} accessible>
          <Text
            style={styles.modalTitlePlaylist}
          >
            {item.name}
          </Text>
          <Radio
